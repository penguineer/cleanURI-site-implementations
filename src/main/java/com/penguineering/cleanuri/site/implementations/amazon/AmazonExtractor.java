...
public class AmazonExtractor implements Extractor {
    ...
    public AmazonExtractor(URI uri) {
        this.uri = uri;
    }

    // Additional constructor for testing
    protected AmazonExtractor(Document doc) {
        this.uri = null; // URI is not needed when a Document is provided
        this.doc = Optional.ofNullable(doc);
    }

    @Override
    public AmazonExtractor withExceptionHandler(BiConsumer<Level, Throwable> exceptionHandler) {
    ...
    public Optional<Pricing> extractPricing() {
        // no pricing information are extracted from the Amazon site
